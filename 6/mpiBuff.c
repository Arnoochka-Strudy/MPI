    ...
    if (isRoot) {
        for (int proc_id = 1; proc_id < ProcNum; ++proc_id) {
            for (int idx = 0; idx < size[0]; ++idx) {
                MPI_Bsend(send_matrix->array[idx + proc_id * size[0]], size[2], MPI_INT, proc_id, idx, MPI_COMM_WORLD);
            }
        }
    } else {
        for (int idx = 0; idx < size[0]; ++idx) {
...
int main(int argc, char* argv[]) {
    ...
    MPI_Init(&argc, &argv);
    int maxNumMatrixesINBuffer = atoi(argv[4]);
    ...
    int buffer_size = (ProcNum * size[0] * size[2] * sizeof(int))*maxNumMatrixesINBuffer;
    void* buffer;
    ...
    for (int k = 0; k < ProcNum; ++k){
        ...
        for (int idx = 0; idx < size[0]; ++idx) {
            ...
            requests = (MPI_Request *)malloc(sizeof(MPI_Request) * 2);
            MPI_Ibsend(send_matrix.array[idx], size[2], MPI_INT, (ProcNum + ProcRank - 1) % ProcNum, idx, MPI_COMM_WORLD, &requests[0]);
            MPI_Irecv(recv_matrix.array[idx], size[2], MPI_INT, (ProcRank + 1) % ProcNum, idx, MPI_COMM_WORLD, &requests[1]);
    ...
    if (isRoot){
        for (int proc_id = 1; proc_id < ProcNum; ++proc_id) {
            for (int idx = 0; idx < size[0]; ++idx) {
                MPI_Recv(result.array[proc_id * size[0] + idx], size[2], MPI_INT, proc_id, idx, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            }
        }
    } else {
        for (int idx = 0; idx < size[0]; ++idx) {
            MPI_Bsend(result.array[idx], size[2], MPI_INT, 0, idx, MPI_COMM_WORLD);
        }
    }
