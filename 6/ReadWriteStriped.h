...
pair_t read_file(char* filename_A, char* filename_B, int mpi_comm_size) {
    ...
    if (n_A % mpi_comm_size != 0) {
        n_A += mpi_comm_size - (n_A % mpi_comm_size);
        n_B += mpi_comm_size - (n_B % mpi_comm_size);
    }


    matrixs.first = init_matrix(n_A, m_A);
    matrixs.second = init_matrix(n_B, m_B);

    read_matrix(&matrixs.first, file_A);
    read_matrix_T(&matrixs.second, file_B);
