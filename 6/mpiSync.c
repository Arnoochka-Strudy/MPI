    ...
    if (isRoot){
        for (int proc_id = 1; proc_id < ProcNum; ++proc_id) {
            for (int idx = 0; idx < size[0]; ++idx) {
                MPI_Ssend(send_matrix->array[idx + proc_id*size[0]], size[2], MPI_INT, proc_id, idx, MPI_COMM_WORLD);
            }
        }
    } else {
        for (int idx = 0; idx < size[0]; ++idx) {
...
int main(int argc, char* argv[]) {
    ...
    for (int k = 0; k < ProcNum; ++k){
        ...
        for (int idx = 0; idx < size[0]; ++idx) {
            ...
            MPI_Issend(send_matrix.array[idx], size[2], MPI_INT, (ProcNum + ProcRank - 1) % ProcNum, idx, MPI_COMM_WORLD, &requests[0]);
    ...
    if (isRoot){
        ...
        for (int idx = 0; idx < size[0]; ++idx) {
            MPI_Ssend(result.array[idx], size[1], MPI_INT, 0, idx, MPI_COMM_WORLD);
        }
    }
